...
const RETAIL_DELIVERY_MINIMUM = 300;
...
export const CheckoutModal: React.FC = () => {
  ...
  const validate = () => {
    ...
    if (
      ...
      subtotal < RETAIL_DELIVERY_MINIMUM
    ) {
      nextErrors.fulfilment = `Retail delivery outside Stellenbosch requires a minimum order of ${formatZAR(RETAIL_DELIVERY_MINIMUM)}.`;
    }
  ...
  const buildOrderSummary = () => {
    const lines = items
      .map(
        (line) =>
          `- *${formatProductName(line.product.name)}* - ${formatQty(line.qty, line.product.unit)} @ ${formatZAR(line.product.price)} = ${formatZAR(line.product.price * line.qty)}`
      )
      .join("\n");
    ...
    if (form.fulfilment === "delivery") {
      ...
      if (orderType === "retail" && isOutsideStellenboschDelivery) {
        detailLines.push(
          `Delivery note: Retail delivery outside Stellenbosch requires a minimum order of ${formatZAR(RETAIL_DELIVERY_MINIMUM)}.`
        );
      }
    }
                ...
                <form onSubmit={submit}>
                  ...
                  <div className="max-h-[60vh] space-y-5 overflow-y-auto p-6 sm:p-8">
                    ...
                    <div>
                      ...
                      {form.fulfilment === "delivery" && orderType === "retail" && (
                        <p className="mt-2 text-xs leading-relaxed text-stone-500">
                          Stellenbosch delivery is free. Kraaifontein, Brackenfell, Durbanville,
                          Joostebergvlakte and Kuils River are R50, and retail delivery outside
                          Stellenbosch starts from {formatZAR(RETAIL_DELIVERY_MINIMUM)}.
                        </p>
                      )}
                    </div>
