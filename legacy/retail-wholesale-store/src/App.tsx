...
import { ShopSwitch, ShopGrid, queueShopFocus, WholesaleFloatingButton } from './shop/Shop';
...
const SELECTION_TARGETS: Record<
  string,
  {
    retail: { category?: string; query?: string };
    wholesale: { category?: string; query?: string };
  }
> = {
  "Premium Beef": {
    retail: { category: "Beef" },
    wholesale: { category: "Beef" },
  },
  "Lamb & Mutton": {
    retail: { category: "Lamb" },
    wholesale: { category: "Lamb" },
  },
  "Quality Chicken": {
    retail: { category: "Chicken" },
    wholesale: { category: "Chicken" },
  },
  "Ostrich Cuts": {
    retail: { category: "Ostrich" },
    wholesale: { category: "Ostrich" },
  },
  "Boerewors & Sausages": {
    retail: { query: "boerewors" },
    wholesale: { query: "boerewors" },
  },
  "Braai Essentials": {
    retail: { query: "braai" },
    wholesale: { query: "braai" },
  },
};
...
const getPreferredCounterMode = (orderType: "retail" | "wholesale") => {
  if (typeof window === "undefined" || orderType !== "wholesale") {
...
const CounterLink: React.FC<{
  children: React.ReactNode;
  className: string;
  retailTarget?: { category?: string; query?: string };
  wholesaleTarget?: { category?: string; query?: string };
}> = ({ children, className, retailTarget, wholesaleTarget }) => {
  const { orderType } = useCart();

  const handleClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
    event.preventDefault();
    const preferredMode = getPreferredCounterMode(orderType);
    const target = preferredMode === "wholesale" ? wholesaleTarget ?? retailTarget : retailTarget;

    queueShopFocus({
      orderType: preferredMode,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((cat, idx) => (
            <motion.button
              ...
              onClick={() => {
                const preferredMode = getPreferredCounterMode(orderType);
                const target = SELECTION_TARGETS[cat.name][preferredMode];

                setOrderType(preferredMode);
                queueShopFocus({
                  orderType: preferredMode,
                  category: target.category,
                  query: target.query,
          ...
          <div className="grid gap-10 xl:grid-cols-[1.05fr_0.95fr] xl:items-end">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg font-light leading-8 text-stone-300">
                ...
                both homes and trade with confidence.
              </p>
              <p className="mt-5 max-w-2xl text-sm leading-7 text-stone-400">
                <CounterLink
                  ...
                  retailTarget={{ category: "Ready to Cook" }}
                  wholesaleTarget={{ category: "Bulk & Pantry" }}
                >
                  Shop now from the live counter
                </CounterLink>{" "}
                or jump into a larger supply order and we will guide you from there based on the
                counter you already have open.
              </p>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-[24px] border border-white/10 bg-stone-950/45 p-5">
                ...
                <p className="text-sm leading-6 text-stone-300">
                  Local Stellenbosch wholesalers and butchers, serving the community since {CONFIG.ESTABLISHED}.
                </p>
              </div>
              <div className="rounded-[24px] border border-white/10 bg-stone-950/45 p-5">
...
const FAQSection = () => {
  const faqs = [
    ...
    {
      question: "How does delivery work for retail orders?",
      answer:
        `Stellenbosch deliveries run Monday to Saturday and orders in by 7:00 AM can qualify for same-day delivery. Stellenbosch delivery is free, Kraaifontein, Brackenfell, Durbanville, Joostebergvlakte and Kuils River are R50 for retail home deliveries, and retail delivery outside Stellenbosch starts from ${formatZAR(CONFIG.RETAIL_DELIVERY_MINIMUM)}. Outside those areas can still be arranged, with possible extra charges depending on the route.`,
    },
    {
          ...
          >
            ...
            <div id="delivery-info" className="mb-10 rounded-[24px] border border-white/10 bg-stone-950/65 px-5 py-5">
              ...
              <p className="mt-3 text-sm leading-7 text-stone-300">
                ...
                Joostebergvlakte and Kuils River retail deliveries are R50, and retail delivery
                outside Stellenbosch starts from {formatZAR(CONFIG.RETAIL_DELIVERY_MINIMUM)}.
              ...
              <p className="mt-3 text-sm leading-7 text-stone-400">
                ...
                and start building the order in the retail or wholesale section you already have
                open. Home deliveries must be paid by EFT before the order is loaded for delivery.
              </p>
            </div>
        ...
        fallback={
          <div className="min-h-screen bg-stone-950 px-6 py-16 text-stone-200">
            <div className="mx-auto max-w-3xl">
              ...
              <h1 className="mt-4 font-serif text-4xl text-stone-100">Loading specials builder...</h1>
            </div>
          </div>
        }
      >
